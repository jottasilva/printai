      ...
      {Object.entries(data).map(([status, count], idx) => (
        <div key={status} className="flex-1 flex flex-col items-center group cursor-help">
          <div className="w-full relative">
            ...
            <div className="absolute -top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap bg-foreground text-white text-[10px] px-2 py-1 rounded shadow-lg font-bold">
              {count} Orders
            </div>
          </div>
          <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mt-4 text-center h-4 flex items-center">
